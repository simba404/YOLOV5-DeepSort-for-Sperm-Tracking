...
# 训练得到的ckpt.pt文件放置在checkpoint文件夹里


parser = argparse.ArgumentParser(description="Train on market1501")
# 准备好的数据路径
parser.add_argument("--data-dir", default='E:\Bristol Project\YOLOv5+DeepSort2.0\yolo_tracking-3.0\prepare\VISEM0.1+SVIA+Bristol_same', type=str)
...
def test(epoch):
    ...
    if acc > best_acc:
        best_acc = acc
        print("Saving parameters to checkpoint/all_same_v5n.t7")
        checkpoint = {
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(checkpoint, './checkpoint/all_same_v5n.t7')
